...
class ALPaCA:
    ...
    def batch_blr(self,X,Y,num):
        X = X[:num,:]
        Y = Y[:num,:]
        Ln_inv = tf.matrix_inverse(tf.matmul(tf.transpose(X), X) + self.L)
        Kn = tf.matmul(Ln_inv, (tf.matmul(tf.transpose(X), Y) + tf.matmul(self.L, self.K)))
    ...
    def train(self, dataset, num_train_updates, config):
        self.config = config
...
def batch_quadform(A, b):
    ...
    if A_dims == b_dims + 1:
        return tf.squeeze( tf.matmul(tf.matmul(tf.matrix_transpose(b_vec) , A) , b_vec), axis=-1)
    elif A_dims == b_dims:
        Ab = tf.expand_dims( tf.matrix_transpose( tf.matmul(A , tf.matrix_transpose(b)) ), axis=-1) # ... x N x n x 1
        return tf.squeeze( tf.matmul(tf.matrix_transpose(b_vec) , Ab), axis = -1) # ... x N x 1
    else:
...
def blr_update_np(K,L,X,Y):
    Ln_inv = np.linalg.inv( tf.matmul(X.T, X) + L )
    Kn = tf.matmul(Ln_inv , (tf.matmul(X.T , Y) + tf.matmul(L , K)))
    return Kn, Ln_inv
...
class AdaptiveDynamics(ALPaCA, object):
    ...
    def linearized_step(self, x, S, u, proc_noise=True,resampling=True):
        ...
        pxK = tf.matmul(tf.matrix_transpose(phi_x) , K)
        pxK_fnomx = pxK + tf.matrix_transpose(f_nom_x)

        Sp = tf.matmul(tf.matmul(dxp_dx , S) , tf.matrix_transpose(dxp_dx)) #tf.matrix_transpose(pxK_fnomx) @ S @ pxK_fnomx
        ...
        if resampling:
            ...
            trace_term = tf.trace(tf.matmul(tf.matmul(tf.matmul(tf.matrix_transpose(phi_x) , Linv) , phi_x) , S))
    ...
    def gen_predict_func(self):
        K = tf.matmul(self.Ln_inv , self.Qn)
        ...
        def predict(x,u):
            ...
            f_nom = f_nom[0,0,:]
            return f_nom + tf.matmul(K.T , phi)
    ...
    def gen_linearize_func(self):
        ...
        def linearize(x,u):
            ...
            for i in range(batch_size):
                s = (1 + tf.matmul(tf.matmul(phi[i,:].T , self.Ln_inv) , phi[i,:]))
                Sx = tf.matmul(tf.matmul(Ap[i,:,:].T , self.Ln_inv) , phi[i,:])
                Su = tf.matmul(tf.matmul(Bp[i,:,:].T , self.Ln_inv) , phi[i,:])
                Sxx = tf.matmul(tf.matmul(Ap[i,:,:].T , self.Ln_inv) , Ap[i,:,:])
                Suu = tf.matmul(tf.matmul(Bp[i,:,:].T , self.Ln_inv) , Bp[i,:,:])
                Sux = tf.matmul(tf.matmul(Bp[i,:,:].T , self.Ln_inv) , Ap[i,:,:])
                ...
                A[i,:,:] = tf.matmul(K.T , Ap[i,:,:])
                B[i,:,:] = tf.matmul(K.T , Bp[i,:,:])
    ...
    def gen_dynamics(self,sample_model=False):
        K = tf.Session().run(tf.matmul(self.Ln_inv , self.Qn))
        ...
        def dynamics(x,u):
            ...
            #     S[0,0] = 1.
            # else:
            #     s = (1 + np.matmul(np.matmul(phi.T , self.Ln_inv) , phi))
                # Sx = tf.matmul(tf.matmul(A.T , self.Ln_inv) , phi)
                # Su = tf.matmul(tf.matmul(B.T , self.Ln_inv) , phi)
                # Sxx = tf.matmul(tf.matmul(A.T , self.Ln_inv) , A)
                # Suu = tf.matmul(tf.matmul(B.T , self.Ln_inv) , B)
                # Sux = tf.matmul(tf.matmul(B.T , self.Ln_inv) , A)
                #
                # S[0,0] = s
                # S[1:1+x_dim,0] = Sx
                # S[0,1:1+x_dim] = Sx.T
            ...
            # Sig = s*self.sigeps # original alpaca code
            s = (1 + np.matmul(np.matmul(phi.T , self.Ln_inv) , phi))
            Sig = s*self.sigeps
            xp = x + np.matmul(K.T, phi)

            # return xp, Sig, S, np.matmul(K.T , A) + np.eye(x_dim), tf.matmul(K.T , B)
            return xp, Sig
    ...
    def error(self, x, u, xp, Ks):
        ...
        xp_preds = np.squeeze( tf.matmul(tp(Ks) , phi), axis=-1 )
    ...
    def sample_dynamics_matrices(self, n_models=10):
        return np.array( [sampleMN(tf.matmul(self.Ln_inv , self.Qn), self.Ln_inv, self.sigeps) for _ in range(n_models)] )
    ...
    def sample_rollout(self, x0, actions, Ks, resample=False):
        ...
        for t in range(0, T):
            ...
            for j in range(N_samples):
                K = Ks[j]
                if resample:
                    K = sampleMN(tf.matmul(self.Ln_inv , self.Qn), self.Ln_inv, self.sigeps)
                A[j,t,:,:] = tf.matmul(K.T , dphi_dxu[j,0,:,:x_dim])
                B[j,t,:,:] = tf.matmul(K.T , dphi_dxu[j,0,:,x_dim:])
                x_pred[j,t+1,:] = x_pred[j,t,:] + np.squeeze( tf.matmul(phi[j,:,:] , K) )

        return x_pred #, A, B

    def reset_to_prior(self):
        self.Ln_inv = np.linalg.inv(self.L0)
        self.Qn = tf.matmul(self.L0 , self.K0)
    ...
    def incorporate_transition(self,x,u,xp):
        ...
        # y = np.reshape(xp - x, (-1,1)) # original alpaca
        f_nom = self.sess.run( self.f_nom_x, {self.x: x_inp} )
        f_nom = f_nom[0,0,:]
        y = np.reshape(xp - f_nom, (-1,1))

        phiLamphi = (np.matmul(np.matmul(phi.T , self.Ln_inv) , phi))[0,0]
        LninvPhi = np.matmul(self.Ln_inv , phi)
        self.Ln_inv = self.Ln_inv - 1./(1. + phiLamphi) * np.matmul(LninvPhi , LninvPhi.T)
        self.Qn = np.matmul(phi , y.T) + self.Qn
    ...
    def incorporate_batch(self, X, Y):
        ...
        self.Ln_inv = Ln_inv
        self.Kn = Kn
        self.Qn = tf.matmul(np.linalg.inv(self.Ln_inv) , self.Kn)
